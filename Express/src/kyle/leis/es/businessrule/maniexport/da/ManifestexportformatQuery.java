...
public class ManifestexportformatQuery extends HGeneralQuery {
	...
	public ManifestexportformatQuery(){
	    m_strSelectClause = "SELECT new kyle.leis.es.businessrule.maniexport.da.ManifestexportformatColumns( mef.mefCode,mef.mefName, mef.mefEname, mef.mefTemplatepath, mef.mefBeginrow, mef.mefBegincolumn, mef.mefCreatedate, mef.mefModifydate, mef.mefExportfilesuffix, opc.opId, opc.opName,   opm.opId,opm.opName,si.ssCode,si.ssName) FROM TbrManifestexportformat as mef inner join mef.tdiOperatorByMefCreator as opc    inner join mef.tdiOperatorByMefModifier as opm  inner join mef.tdiSimplestatus as si";
	    m_strWhereClause = "";
	    m_strOrderByClause = "";
	    m_strGroupByClause = "";
	    m_astrConditionWords = new String[] { "mef.mefCode = '~~'", "mef.mefName like '%~~%'", "opc.opId = '~~'", "opc.opName = '~~'", "opm.opId = '~~'", "opm.opName = '~~'", "si.ssCode = '~~'", "mef.mefCreatedate >= to_date('~~','yyyy-mm-dd hh24:mi:ss')", "to_date('~~','yyyy-mm-dd hh24:mi:ss') >= mef.mefCreatedate", "mef.mefModifydate >= to_date('~~','yyyy-mm-dd hh24:mi:ss')", "to_date('~~','yyyy-mm-dd hh24:mi:ss') >= mef.mefModifydate" };
