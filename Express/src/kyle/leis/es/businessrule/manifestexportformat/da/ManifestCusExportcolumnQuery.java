...
public class ManifestCusExportcolumnQuery extends JGeneralQuery {
	...
	public ManifestCusExportcolumnQuery(String cusSql,String customerewbcode,String serverewbcode){
	    ...
	    if(!StringUtility.isNull(customerewbcode)){
	    	String[] cwcode=customerewbcode.split(",");
	    	String sqlcwcode="";
	    	for(String s:cwcode)
	    		sqlcwcode=sqlcwcode+","+"'"+s+"'";
	    	//去掉第一个逗号
	    	sqlcwcode=sqlcwcode.substring(1,sqlcwcode.length());
	    	m_strWhereClause=m_strWhereClause+"and cw.cw_customerewbcode in ("+sqlcwcode+")";
	    }
	    if(!StringUtility.isNull(serverewbcode)){
	    	String[] swcode=serverewbcode.split(",");
	    	String sqlswcode="";
	    	for(String s:swcode)
	    		sqlswcode=sqlswcode+","+"'"+s+"'";
	    	sqlswcode=sqlswcode.substring(1,sqlswcode.length());
	    	m_strWhereClause=m_strWhereClause+"and cw.cw_serverewbcode in("+sqlswcode+")";
	    }
	    m_strOrderByClause = "";
	    m_strGroupByClause = "";
	...
	public IColumns createColumns() {
		...
		return  new ManifestCusExportcolumnColumns(columnsSize);
	}
