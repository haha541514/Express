...
public class CustomerpricegroupQuery extends HGeneralQuery {
	
	public CustomerpricegroupQuery(){
	    m_strSelectClause = "SELECT new kyle.leis.es.price.pricegroup.da.CustomerpricegroupColumns(cpg.epCode,ep.epCreatedate,ep.epModifydate,ep.epStartdate,ep.epEnddate,ep.epRemark,ep.epWithdrawsign,ps.psCode,ps.psName,epk.epkCode,epk.epkName,cop.opId,cop.opName,mop.opId,mop.opName,ee.eeCode,ee.eeSname,ee.eeEsname,co.coCode,co.coSname,co.coSename,co.coLabelcode,cst.cstCode,cst.cstName,pgk.pgkCode,pgk.pgkName,pk.pkCode,pk.pkSname,pk.pkSename) FROM TepCustomerpricegroup as cpg inner join cpg.tepExpressprice as ep inner join ep.tdiPricestatus as ps inner join ep.tdiExpresspricekind as epk inner join ep.tdiOperatorByEpOpIdCreate as cop inner join ep.tdiOperatorByEpOpIdModify as mop inner join ep.tdiEnterpriseelement as ee inner join cpg.tcoCorporation as co left join cpg.tdiPricegroupkind as pgk inner join cpg.tdiProductkind as pk inner join co.tdiCustomersuppliertype as cst";
	    m_strWhereClause = "epk.epkCode = 'A05'";
	    m_strOrderByClause = "";
	    m_strGroupByClause = "";
	    m_astrConditionWords = new String[] { "ep.epCode = ~~", "ep.epEnddate >= to_date('~~','yyyy-mm-dd hh24:mi:ss')", "to_date('~~','yyyy-mm-dd hh24:mi:ss') >= ep.epStartdate", "ep.epCreatedate >= to_date('~~','yyyy-mm-dd hh24:mi:ss')", "to_date('~~','yyyy-mm-dd hh24:mi:ss') >= epCreatedate", "ep.epModifydate >= to_date('~~','yyyy-mm-dd hh24:mi:ss')", "to_date('~~','yyyy-mm-dd hh24:mi:ss') >= ep.epModifydate", "ps.psCode = '~~'", "cop.opId = ~~", "mop.opId = ~~", "pk.pkCode = '~~'", "pgk.pgkCode = '~~'", "co.coCode = '~~'", "epk.epkCode = '~~'", "ee.eeCode = '~~'" };
