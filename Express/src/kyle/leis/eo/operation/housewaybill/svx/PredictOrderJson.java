...
public class PredictOrderJson {
private String m_shipment;
	
	public PredictOrderJson(String shipment) {
		m_shipment = shipment;
	}
	
	public PredictOrderColumnsEX getPredictOrderEX(){
		if (StringUtility.isNull(m_shipment))
			return null;
...
//		pwbc.setPwbcw_code(wfpd.getCwcw_code());
	...
	private WaybillforpredictColumns getPredictwaybillColumns(List<CorewaybillpiecesColumns> listPieces){
		...
		JSONObject result =  JSONObject.fromObject(m_shipment);
	...
	private List<CorewaybillpiecesColumns> getCorewaybillpiecesColumns() {
		...
		JSONObject pieces =  JSONObject.fromObject(shipmentDetails.get("Pieces"));
		JSONArray pieceList = JSONArray.fromObject(pieces.get("Piece"));
	...
	private List<CargoinfoColumns> getPredictcargoinfoColumns(){
		...
		JSONObject result =  JSONObject.fromObject(m_shipment);
		JSONObject shipmentDetails =  JSONObject.fromObject(result.get("ShipmentDetails"));
		JSONObject contents =  JSONObject.fromObject(shipmentDetails.get("Contents"));
