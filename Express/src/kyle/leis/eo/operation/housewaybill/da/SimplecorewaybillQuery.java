...
public class SimplecorewaybillQuery extends JGeneralQuery {
	...
	public SimplecorewaybillQuery(){
	    m_strSelectClause = "SELECT cw.CW_CODE,cw.CWS_CODE,cw.CT_CODE,cw.PM_CODE,cw.PK_CODE,cw.IHS_CODE,cw.CO_CODE_CUSTOMER,cw.CO_CODE_SUPPLIER,cw.Chn_Code_Supplier,cw.cw_createdate FROM t_op_corewaybill cw";
	    m_strWhereClause = "";
	    m_strOrderByClause = "";
	    m_strGroupByClause = "";
	    m_astrConditionWords = new String[] { "cw.CW_CODE = ~~", "cw.CW_CUSTOMEREWBCODE = '~~'", "cw.CW_SERVEREWBCODE = '~~'", "cw.CW_EWBCODE = '~~'", "cw.Chn_Code_Supplier = '~~'", "cw.co_code_customer = '~~'", "cw.cw_createdate >= to_date('~~','yyyy-mm-dd hh24:mi:ss')", "to_date('~~','yyyy-mm-dd hh24:mi:ss') >= cw.cw_createdate", "cw.cws_code not in (~~)", "exists (select cwp.cw_code from t_op_corewaybillpieces cwp where cwp.cw_code and cwp.cp_labelcode = '~~')" };
