...
public class HousewaybillfortarazzQuery extends JGeneralQuery {
	
	public HousewaybillfortarazzQuery(){
	    m_strSelectClause = "SELECT schn.chn_sname as channelcode,cw.cw_customerewbcode as ReferenceId,cw.cw_serverewbcode as ShipmentId,hw.hw_consigneename as ReceiverAttn, hw.hw_consigneecompany as ReceiverCompany, hw.hw_consigneeaddress1 as  ReceiverAdd1,hw.hw_consigneeaddress2 as ReceiverAdd2, hw.hw_consigneeaddress3 as ReceiverAdd3, '' as ReceiverAdd4, hw.hw_consigneecity as ReceiverCity, ddt.dt_hubcode as ReceiverCityCode,hw.hw_consigneepostcode as ReceiverPostalCode, hw.hw_consigneetelephone as ReceiverMobile, '' as ReceiverEmail, FUN_GET_CARGOHSCODE(cw.cw_code) as HSCode, FUN_GET_CARGOINFO(cw.cw_code) as CargoInfo,FUN_GET_VolumeWeight(cw.cw_code) as VolumeWeight, FUN_GET_LENGTH(cw.cw_code) as Length,FUN_GET_WIDTH(cw.cw_code) as Width, FUN_GET_HEIGHT(cw.cw_code) as Height,cw.cw_transferchargeweight as ShipmentWeight,FUN_GET_UNITCARGO(cw.cw_code) as UnitCargoInfo,FUN_GET_TOTALCARGOINFO(cw.cw_code) as TotalCargoInfo FROM T_OP_HOUSEWAYBILL hw,T_OP_COREWAYBILL cw,T_DI_COREWAYBILLSTATUS cws,T_DI_ENTERPRISEELEMENT ee,T_DI_PRODUCTKIND pk,T_DI_DISTRICT ddt,T_DI_DISTRICT cddt, T_DI_DISTRICT sdt, T_DI_DISTRICT odt,T_DI_PAYMENTMODE pm,T_CHN_CHANNEL schn,T_CHN_CHANNEL cchn, T_DI_CARGOTYPE ct,T_CO_CORPORATION sco, T_CO_CORPORATION cco, T_CO_CUSTOMER cm,t_co_financialstatistics cofs,T_DI_CUSTOMERTYPE cct,T_DI_OPERATOR csop,T_DI_OPERATOR ssop,T_DI_OPERATOR siop,T_DI_OPERATOR soop,T_DI_OPERATOR rcop,T_OP_BATCHWAYBILL abw,T_OP_BATCHWAYBILL dbw,T_DI_ISSUEHOLDSTATUS ihs";
